...
def resize_image(image, size, letterbox_image):
    ...
    if letterbox_image:
        ...
        new_image = np.zeros((w, h, 3), dtype='float32')
        top, left = (w - nw) // 2, (h - nh) // 2
        new_image[left:w-left, top:h-top] = image
    else:
        #new_image = image.resize((w, h), Image.BICUBIC)
        new_image = cv2.resize(image, (w, h), cv2.INTER_CUBIC)
    return new_image

#---------------------------------------------------#
#   对输入图像进行resize
#---------------------------------------------------#
def resize_image1(image, size, letterbox_image):
    iw, ih = image.size
    w, h    = size
    #ih, iw, _ = image.shape
    #h, w = size
